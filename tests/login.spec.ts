...
test('Test login with invalid credentials', async({ page }) => {
  //Fill username field
  await page.locator('input[name="username"]').fill('ozlem');
  // Fill password field
  await page.locator('input[name="password"]').fill('incorrect');
  // Click on Submit button 
  await page.locator('input[value="SUBMIT"]').click();
  //Check Page URL
  await expect(page).not.toHaveURL('http://php.testsparker.com/auth/internal.php');
});

test('Test login without entering credentials', async({ page }) => {
  // Click on Submit button 
  await page.locator('input[value="SUBMIT"]').click();
  //Check Page URL
  await expect(page).not.toHaveURL('http://php.testsparker.com/auth/internal.php');
